...
module.exports = function(context) {
     ...
     * @param {ASTNode[]} items The sequence of nodes to check.
     * @param {String|RegExp} checkOperator The operator(s) for which spacing checks should be applied
     * @returns {void}
     * @private
     */
    function checkList(items, checkOperator) {
        for (var i = 0, l = items.length; i < l; i++) {
            var left = items[i - 1],
                right = items[i],
                operator = context.getTokenBefore(right);

            if (operator && operator.type === "Punctuator" && operator.value.match(checkOperator)) {
    ...
    /**
     * Report if there are multiple spaces around items in function parameter specifications,
     * including default values.
     * @param {ASTNode} node The function declaration/expression node.
     * @returns {void}
     * @private
     */
    function checkFunctionParameters(node) {
        ...
        for (var i = 0, l = node.params.length; i < l; i++) {
            items.push(node.params[i]);
            if (node.defaults[i]) {
                items.push(node.defaults[i]);
            }
        }
        checkList(items, /[,=]/);
    }
